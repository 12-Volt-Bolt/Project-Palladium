...
public class VisionAuto extends CommandGroup {
	...
	public VisionAuto() {
		...
		addSequential(new DriveCommand(0.61, 0.61, SmartDashboard.getNumber("FIRST_DRIVE", 2.2)));
		// Turn to face the tower
		addSequential(new DriveCommand(0.5, -0.5, SmartDashboard.getNumber("TURN", 0.4)));
		// Confirm alignment with tower
		addSequential(new TrackGoalCommand());
		// Drive to batter
		addSequential(new DriveCommand(0.5, 0.5, SmartDashboard.getNumber("DRIVE_TO_BATTER", 0.4)));
		// Confirm alignment one last time.
		addSequential(new TrackGoalCommand());
		// Drive up batter.
		addSequential(new DriveCommand(0.25, 0.25, SmartDashboard.getNumber("DRIVE_UP_BATTER", 1)));
